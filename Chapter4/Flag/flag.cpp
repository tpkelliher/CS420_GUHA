...
// Number of segments along the  length (i.e., sine curve section) of the flag.
static int p = 20;
...
void fillVertexArray(void)
{
    ...
    for (int j = 0; j <= q; j++)
        for (int i = 0; i <= p; i++)
        {
            ...
            vertices[k++] = 5.0 * sin(s + (float)i / p * 1.8 * M_PI)
                - 5.0 * sin(s);
...
void setup(void)
{
    ...
    // Array allocation with values of p and q. 
...
void specialKeyInput(int key, int x, int y)
{
    if (key == GLUT_KEY_DOWN)
        animationPeriod += 5;
    if (key == GLUT_KEY_UP && animationPeriod > 5)
        animationPeriod -= 5;
    glutPostRedisplay();
}
